"""
Experiment 3: Flair NER
Usage: python test_flair.py [file_path]
"""
...
RESULTS_DIR = Path("results/03_flair")
...
tagger_en = SequenceTagger.load("ner")
tagger_fr = SequenceTagger.load("flair/ner-french")
taggers = {"en": tagger_en, "fr": tagger_fr}
print("Done.\n")

all_results = []
for doc_name, doc in TEST_DOCS.items():
    print(f"\n{'='*60}\n  Document: {doc_name}  ({doc['lang']})\n{'='*60}")
    tagger = taggers[doc["lang"]]
    sentence = Sentence(doc["text"])
    start = time.time()
    tagger.predict(sentence)
    elapsed = time.time() - start
    entities = [{"text": ent.text, "type": ent.tag, "start": ent.start_position, "end": ent.end_position} for ent in sentence.get_spans("ner")]
    for e in entities:
...
with open(RESULTS_DIR / "flair_results.json", "w", encoding="utf-8") as f:
    json.dump(all_results, f, indent=2, ensure_ascii=False)
print(f"\nResults -> {RESULTS_DIR / 'flair_results.json'}")
